...
void ThreadIRCSeed(void* parg)
{
    ...
    RenameThread("SmurfsCoin-ircseed");
...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #SmurfsCoinTEST2\r");
            Send(hSocket, "WHO #SmurfsCoinTEST2\r");
        } else {
            // randomly join #SmurfsCoin00-#SmurfsCoin05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #SmurfsCoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #SmurfsCoin%02d\r", channel_number).c_str());
        }
