SmurfsCoin-qt: Qt4 GUI for SmurfsCoin
===============================
...
Alternatively, install Qt Creator and open the `SmurfsCoin-qt.pro` file.

An executable named `SmurfsCoin-qt` will be built.
...
A warning for people using the *static binary* version of SmurfsCoin on a Linux/UNIX-ish system (tl;dr: **Berkely DB databases are not forward compatible**).

The static binary version of SmurfsCoin is linked against libdb5.3.
...
installed causes SmurfsCoin-qt to crash intermittently.  The issue has been reported as `launchpad bug 857790`_, but
isn't yet fixed.
